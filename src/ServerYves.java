...
public class ServerYves {
	 ...
	 */
	ServerYves(ServerSocket serverSocket) {
		...
		public void run() {
			...
			if (increment == 0) { // Si on arrive a zero, on renvoie le
				...
				if (resultats.get(increment) == null) {// On verifie si le
														...
														// presents. Si oui, on
														// le calcule.
					ClientYves client = new ClientYves(increment - 1, serverSocket.getLocalPort());
					client.clientRun();
	...
	public static void main(String[] args) {
		...
		try {// Un nouveau serveur avec comme parametre le port donne lors de la
			...
			ServerYves server = new ServerYves(serverSocket);
